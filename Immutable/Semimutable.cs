...
{
    /// <summary>
    /// Represents a "semi mutable" value. Its value can be changed either via the setter of the <see cref="Value"/>
    ...
    /// never change again.
    /// <para>
    /// It's like Schrodinger's cat - once you open the box, the cat's fate is sealed.
    /// </para>
    /// </summary>
    ...
    public class Semimutable<T>
    {
        ...
        /// </summary>
        public bool IsLocked => _lockedInstance != null;
        ...
        public void SetValue(Func<T> getValue)
        {
            ...
            while (_lockedInstance == null)
            {
                ...
                if (!_softLock.TryAcquire()) continue;
        ...
        private T GetValue()
        {
            // In the rare case that _lockedInstance is cleared (via the UnlockValue method)
            // after the while loop's null check and before its Value property is access,
            // capture _lockedInstance in a local variable to prevent a null reference exception.
            Lazy<T> local;
